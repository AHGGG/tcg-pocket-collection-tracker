    ...
    <article className={`review-row ${decision.selected ? 'selected' : ''}`}>
      <header>
        ...
        <span className="muted">
          {group.firstSeen.toFixed(1)}–{group.lastSeen.toFixed(1)}s · {group.sightings} sighting(s), not copies
        </span>
      </header>
      ...
      <div className="review-fields">
        ...
        <p>
          Baseline: <strong>{previous?.quantity ?? 'unknown'}</strong>
        </p>
      </div>
      {group.internalId === null && <p className="warning-text">Card matching was ambiguous. Similarity is not a probability.</p>}
      {group.quantities.length > 1 && (
        <p className="warning-text">Conflicting counts: {group.quantities.join(', ')}. Check the source; no count was chosen automatically.</p>
      )}
      {group.hasLowerBound && (
        <p className="warning-text">A capped count such as 99+ was seen. Enter an independently verified exact count or exclude this row.</p>
      )}
      ...
      {decreases && (
        <label className="check-label warning-text">
          <input type="checkbox" checked={decision.decreaseApproved} onChange={(event) => onChange({ ...decision, decreaseApproved: event.target.checked })} />I
          verified this decrease from {previous.quantity} to {decision.quantity}.
        </label>
      )}
      ...
      <div className="evidence-strip">
        {group.evidence.map((observation) => (
          <figure key={observation.key}>
            ...
            <figcaption>
              {observation.timestamp.toFixed(1)}s:{' '}
              {observation.quantity.kind === 'unknown' ? 'unreadable' : `${observation.quantity.value}${observation.quantity.kind === 'at-least' ? '+' : ''}`}
            </figcaption>
          </figure>
        ))}
        {card && (
