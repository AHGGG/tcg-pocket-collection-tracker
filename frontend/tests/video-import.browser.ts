...
const render = () => {
  ...
  if (output) { output.textContent = `${results.passed} passed, ${results.failed} failed\n${results.messages.join('\n')}` }
}
const assert = (condition: unknown, message: string) => {
  if (!condition) { throw new Error(message) }
}
async function check(name: string, work: () => void | Promise<void>) {
  try { await work(); results.passed++; results.messages.push(`PASS ${name}`) }
  catch (error) { results.failed++; results.messages.push(`FAIL ${name}: ${String(error)}`) }
  render()
}
...
await check('incorrect teaching labels fail rather than guessing digit segmentation', () => {
  let failed = false
  try { reader.learn(digitImage('12', false), '12345', 'dark') } catch { failed = true }
...
await check('clipped crop is rejected', () => {
  ...
  let failed = false
  try { cropCanvas(canvas, { x: 19, y: 0, width: 4, height: 4 }) } catch { failed = true }
...
await check('native video decoder seeks to distinct frames and repeated timestamps', async () => {
  ...
  let failed = false
  try { await recording.frame(0) } catch { failed = true }
...
await check('cancel before opening never starts video decoding', async () => {
  ...
  let aborted = false
  try { await openRecording(file, controller.signal) } catch (error) { aborted = error instanceof DOMException && error.name === 'AbortError' }
  assert(aborted, 'abort was not honored')
})
await check('corrupt video rejects without producing a collection', async () => {
  let failed = false
  try { await openRecording(new File(['not a video'], 'bad.mp4', { type: 'video/mp4' })) } catch { failed = true }
